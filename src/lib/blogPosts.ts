// Blog Posts Data
// This file contains all blog posts for the Eyesite Opticians website

export interface BlogPost {
  slug: string;
  title: string;
  excerpt: string;
  content: string;
  author: string;
  date: string;
  category: string;
  image: string;
  readTime: string;
  tags: string[];
}

export const blogPosts: BlogPost[] = [
  {
    slug: "how-often-should-you-get-eye-test",
    title: "How Often Should You Get an Eye Test?",
    excerpt: "Regular eye tests are crucial for maintaining good vision and detecting potential health issues early. Learn how often you should schedule your eye exams.",
    content: `
# How Often Should You Get an Eye Test?

Regular eye examinations are essential for maintaining good eye health and detecting potential problems early. But how often should you actually get your eyes tested?

## General Guidelines

### Adults (18-60 years)
If you're an adult with no known vision problems or risk factors, you should have an eye test **at least every two years**. This is the recommendation from the College of Optometrists and the NHS.

### Over 60s
Once you reach 60, it's recommended to have annual eye tests. As we age, we become more susceptible to eye conditions such as:
- Cataracts
- Glaucoma
- Age-related macular degeneration (AMD)
- Diabetic retinopathy

### Children
Children should have their eyes tested:
- Before starting school (around age 4-5)
- Every year during their school years
- More frequently if they wear glasses or have vision problems

## When to Get Tested More Frequently

You should have more frequent eye tests if you:

### Have Existing Eye Conditions
If you wear glasses or contact lenses, or have been diagnosed with an eye condition, your optometrist will recommend how often you should be tested.

### Have Diabetes
Diabetic patients should have annual eye screenings as diabetes can affect the blood vessels in your eyes, leading to diabetic retinopathy.

### Have a Family History
If close family members have had glaucoma or other eye diseases, you may need more frequent tests.

### Work with Screens
If you spend long hours working on computers or digital devices, regular eye tests can help detect early signs of digital eye strain.

## NHS Free Eye Tests

You're entitled to a free NHS eye test if you:
- Are under 16 (or under 19 and in full-time education)
- Are 60 or over
- Have diabetes or glaucoma
- Are 40 or over and have a close relative with glaucoma
- Are registered as partially sighted or blind
- Receive certain benefits

## Warning Signs Between Tests

Book an eye test sooner if you experience:
- Sudden vision changes
- Eye pain or discomfort
- Frequent headaches
- Difficulty seeing at night
- Floaters or flashes of light
- Double vision

## What Happens During an Eye Test?

At Eyesite Opticians, a comprehensive eye examination includes:

1. **Visual Acuity Test** - Testing how well you see at various distances
2. **Refraction Test** - Determining your exact prescription
3. **Eye Health Check** - Examining the health of your eyes
4. **Pressure Test** - Checking for signs of glaucoma
5. **OCT Scan** - Advanced imaging of the back of your eye (recommended)

## Book Your Eye Test Today

Don't wait until you notice problems with your vision. Regular eye tests can detect issues before you're even aware of them.

At Eyesite Opticians in Leeds, we offer:
- Comprehensive eye examinations
- NHS and private appointments
- Advanced OCT scanning technology
- Expert advice on eye health
- Wide range of eyewear solutions

**Call us today on 0113 260 0063** to book your appointment.
    `,
    author: "Dr. Sarah Mitchell",
    date: "2024-11-15",
    category: "Eye Health",
    image: "/eye-exams.png",
    readTime: "5 min read",
    tags: ["Eye Tests", "Eye Health", "NHS", "Prevention"]
  },
  {
    slug: "choosing-perfect-glasses-for-your-face-shape",
    title: "Choosing the Perfect Glasses for Your Face Shape",
    excerpt: "Finding the right frames can enhance your features and boost your confidence. Discover which glasses suit your face shape best.",
    content: `
# Choosing the Perfect Glasses for Your Face Shape

Selecting the perfect pair of glasses is about more than just vision correction – it's a fashion statement that can enhance your natural features. Here's our comprehensive guide to finding frames that suit your face shape.

## Determining Your Face Shape

Before choosing frames, identify your face shape by looking in a mirror:

### Oval Face
- Balanced proportions
- Slightly wider at the cheekbones
- Forehead slightly wider than chin

### Round Face
- Full cheeks
- Rounded chin
- Similar width and length

### Square Face
- Strong jawline
- Broad forehead
- Angular features

### Heart-Shaped Face
- Wider forehead
- Narrow chin
- Prominent cheekbones

### Diamond Face
- Narrow forehead and jaw
- Wide cheekbones
- Angular features

### Rectangle/Oblong Face
- Longer than wide
- Straight cheek line
- Long nose

## Best Frames for Each Face Shape

### Oval Face - The Lucky One
Oval faces are the most versatile and suit almost any frame style!

**Best choices:**
- Oversized frames
- Geometric shapes
- Cat-eye styles
- Aviators

**Avoid:** Frames that are too large and overwhelm your features

### Round Face - Add Definition
**Best choices:**
- Rectangular frames
- Angular styles
- Cat-eye frames
- Geometric designs

**Key tip:** Look for frames wider than the broadest part of your face

**Avoid:** Round or circular frames that emphasize roundness

### Square Face - Soften Angles
**Best choices:**
- Round frames
- Oval shapes
- Cat-eye styles
- Rimless or light-colored frames

**Key tip:** Choose frames with curves to balance angular features

**Avoid:** Rectangular or geometric frames

### Heart-Shaped Face - Balance the Width
**Best choices:**
- Bottom-heavy frames
- Round or oval shapes
- Aviators
- Rimless frames

**Key tip:** Choose frames that are wider at the bottom

**Avoid:** Top-heavy or oversized frames

### Diamond Face - Highlight Eyes
**Best choices:**
- Cat-eye frames
- Oval shapes
- Rimless styles
- Frames with detailing on brow line

**Key tip:** Choose frames that highlight your eyes and brow

**Avoid:** Narrow frames

### Rectangle Face - Add Width
**Best choices:**
- Large, decorative frames
- Round or square shapes
- Deep frames

**Key tip:** Choose frames that add width to balance length

**Avoid:** Narrow rectangular frames

## Additional Considerations

### Skin Tone
- **Warm tones** (golden, peachy): Tortoiseshell, gold, olive, brown
- **Cool tones** (pink, blue): Black, silver, blue, pink, purple
- **Neutral tones**: Most colors work well!

### Hair Color
- **Dark hair**: Bold, vibrant colors
- **Light hair**: Softer, lighter tones
- **Red hair**: Greens, browns, golds

### Lifestyle
- **Active lifestyle**: Durable materials, secure fit
- **Professional**: Classic, understated styles
- **Fashion-forward**: Bold, trendy designs

## Designer Collections at Eyesite Opticians

We stock premium designer frames including:
- **Ray-Ban** - Timeless classics
- **Tom Ford** - Luxury sophistication
- **Gucci** - Bold fashion statements
- **Saint Laurent** - Parisian elegance
- **Hugo Boss** - Professional style
- **Jimmy Choo** - Glamorous designs
- **Ted Baker** - British contemporary style

## Expert Fitting Service

At Eyesite Opticians, our expert team will:
1. Analyse your face shape
2. Consider your lifestyle needs
3. Match frames to your colouring
4. Ensure perfect fit and comfort
5. Help you find your signature style

## Visit Our Showroom

Experience our extensive collection of designer frames at our Leeds showroom. Our friendly opticians will guide you through the selection process and help you find frames that make you look and feel fantastic.

**Book your eyewear consultation: 0113 260 0063**

Visit us at 198 Selby Rd, Halton, Leeds LS15 0LF
    `,
    author: "Emily Roberts",
    date: "2024-11-20",
    category: "Eyewear",
    image: "/eyewear.png",
    readTime: "6 min read",
    tags: ["Eyewear", "Fashion", "Style Guide", "Designer Frames"]
  },
  {
    slug: "understanding-dry-eye-syndrome",
    title: "Understanding Dry Eye Syndrome: Causes and Treatments",
    excerpt: "Dry eye syndrome affects millions of people. Learn about the causes, symptoms, and effective treatments available.",
    content: `
# Understanding Dry Eye Syndrome: Causes and Treatments

Dry eye syndrome is one of the most common eye conditions, affecting millions of people worldwide. If you've experienced scratchy, burning, or watery eyes, you might be suffering from this condition.

## What is Dry Eye Syndrome?

Dry eye syndrome occurs when your eyes don't produce enough tears or when your tears evaporate too quickly. Tears are essential for:
- Keeping eyes lubricated
- Washing away debris
- Protecting against infection
- Maintaining clear vision

## Common Symptoms

### You might have dry eye syndrome if you experience:
- Scratchy or gritty feeling in your eyes
- Burning or stinging sensation
- Redness
- Watery eyes (paradoxically)
- Blurred vision that improves with blinking
- Eye fatigue
- Difficulty wearing contact lenses
- Light sensitivity

## Main Causes

### Age
As we age, tear production naturally decreases. People over 50 are more likely to experience dry eyes.

### Screen Time
Extended use of computers, tablets, and smartphones reduces blink rate, leading to tear evaporation.

### Environmental Factors
- Air conditioning and heating
- Wind and dry climates
- Smoke and pollution
- High altitudes

### Medical Conditions
- Diabetes
- Rheumatoid arthritis
- Thyroid disorders
- Sjögren's syndrome
- Meibomian gland dysfunction

### Medications
Certain medications can reduce tear production:
- Antihistamines
- Decongestants
- Antidepressants
- Blood pressure medications
- Hormone replacement therapy

### Contact Lenses
Long-term contact lens wear can contribute to dry eye symptoms.

## Types of Dry Eye

### Aqueous Deficient
Your eyes don't produce enough of the watery component of tears.

### Evaporative
Tears evaporate too quickly due to meibomian gland dysfunction (blocked oil glands).

### Mixed
A combination of both types (most common).

## Diagnosis

At Eyesite Opticians, our comprehensive dry eye assessment includes:

1. **Symptom Questionnaire** - Understanding your experience
2. **Tear Break-Up Time** - Measuring tear film stability
3. **Schirmer's Test** - Assessing tear production
4. **Meibomian Gland Evaluation** - Checking oil gland function
5. **Corneal Staining** - Detecting surface damage

## Treatment Options

### Lifestyle Modifications

**Take Screen Breaks**
Follow the 20-20-20 rule: Every 20 minutes, look at something 20 feet away for 20 seconds.

**Increase Humidity**
Use a humidifier, especially in winter or air-conditioned environments.

**Protect Your Eyes**
Wear wraparound sunglasses outdoors to reduce tear evaporation.

**Stay Hydrated**
Drink plenty of water throughout the day.

### Over-the-Counter Solutions

**Artificial Tears**
Lubricating eye drops provide immediate relief. Use preservative-free drops if applying more than 4 times daily.

**Eye Ointments**
Thicker than drops, ideal for nighttime use.

**Warm Compresses**
Help unblock oil glands and improve tear quality.

### Professional Treatments at Eyesite Opticians

**Intense Pulsed Light (IPL) Therapy**
Advanced treatment targeting meibomian gland dysfunction and inflammation.

**Prescription Eye Drops**
- Anti-inflammatory drops (Cyclosporine, Lifitegrast)
- Steroid drops for severe cases

**Punctal Plugs**
Tiny devices inserted into tear ducts to prevent drainage, keeping tears on the eye surface longer.

**BlephEx Treatment**
Deep cleaning of eyelid margins to remove debris and bacteria.

**Nutritional Supplements**
Omega-3 fatty acids (EPA and DHA) can improve tear quality.

## Prevention Tips

1. **Blink More Often** - Especially when using screens
2. **Position Screens Correctly** - Below eye level to reduce eye exposure
3. **Take Regular Breaks** - Step away from screens
4. **Clean Eyelids** - Daily lid hygiene routine
5. **Omega-3 Rich Diet** - Include fish, flaxseed, and walnuts
6. **Manage Allergies** - Control underlying conditions
7. **Review Medications** - Discuss alternatives with your doctor

## When to Seek Help

See an optometrist if you experience:
- Persistent symptoms affecting daily life
- Vision changes
- Severe pain
- Symptoms not responding to over-the-counter treatments

## Our Dry Eye Clinic

At Eyesite Opticians, we specialise in dry eye management. Our services include:

- Comprehensive dry eye assessment
- Personalised treatment plans
- Modern IPL therapy
- Ongoing support and monitoring
- Latest treatment options

Don't suffer in silence. Effective treatments are available!

**Book your dry eye consultation: 0113 260 0063**

Located at 198 Selby Rd, Halton, Leeds LS15 0LF
    `,
    author: "Dr. James Patterson",
    date: "2024-11-25",
    category: "Eye Conditions",
    image: "/dry eye.png",
    readTime: "8 min read",
    tags: ["Dry Eye", "Treatment", "Eye Health", "IPL Therapy"]
  },
  {
    slug: "contact-lenses-guide-for-beginners",
    title: "Contact Lenses: A Complete Guide for Beginners",
    excerpt: "Thinking about switching to contact lenses? This comprehensive guide covers everything you need to know to get started.",
    content: `
# Contact Lenses: A Complete Guide for Beginners

Considering making the switch from glasses to contact lenses? Whether for sports, convenience, or cosmetic reasons, contact lenses offer many benefits. Here's everything you need to know to get started.

## Why Choose Contact Lenses?

### Freedom and Convenience
- No frames obstructing your vision
- No fogging up in cold weather
- No rain droplets or smudges
- Perfect for sports and active lifestyles

### Natural Vision
- Wider field of view
- No distortion at the edges
- Better peripheral vision
- More natural eye movement

### Cosmetic Benefits
- Change your eye colour (coloured lenses)
- No frames covering your face
- Freedom to wear non-prescription sunglasses

## Types of Contact Lenses

### Based on Replacement Schedule

**Daily Disposable**
- Wear once and throw away
- No cleaning required
- Most hygienic option
- Ideal for occasional wear
- Perfect for beginners

**Monthly Disposable**
- Wear for up to 30 days
- Require nightly cleaning
- More cost-effective for daily wear
- Good for regular users

**Two-Weekly Disposable**
- Replace every 14 days
- Balance between convenience and cost
- Popular choice for many wearers

### Based on Lens Type

**Soft Lenses**
- Most comfortable
- Easy to adapt to
- Best for beginners
- Available in various materials

**Rigid Gas Permeable (RGP)**
- Sharper vision
- More durable
- Better for some prescriptions
- Longer adaptation period

### Specialised Lenses

**Toric Lenses**
For astigmatism correction
- Weighted design
- Specific orientation
- Available in daily and monthly

**Multifocal Lenses**
For presbyopia (age-related focusing difficulty)
- Multiple prescription zones
- Alternative to reading glasses
- Various designs available

**Colored Lenses**
For cosmetic purposes
- Enhancement tints
- Opaque colors
- Available with or without prescription

## The Fitting Process at Eyesite Opticians

### Initial Consultation
1. **Eye Examination** - Checking your prescription and eye health
2. **Lifestyle Discussion** - Understanding your needs and preferences
3. **Lens Selection** - Choosing the right type for you
4. **Measurements** - Determining correct lens size and curvature

### Trial Period
- Try sample lenses in practice
- Learn insertion and removal
- Practice care routine
- Wear schedule guidance

### Follow-Up Care
- Check-up after first week
- Assessment after one month
- Regular annual check-ups
- Ongoing support and advice

## Learning to Handle Contact Lenses

### Insertion Tips for Beginners

1. **Wash Your Hands** - Always use soap and water
2. **Check the Lens** - Ensure it's not inside out
3. **Use Your Dominant Hand** - Hold lens on index finger
4. **Hold Eyelids Open** - Use other hand to prevent blinking
5. **Look Up** - Place lens below pupil
6. **Close Eye Gently** - Allow lens to centre
7. **Blink** - Settles the lens in place

**Pro tip:** Practice with daily disposables first!

### Removal Techniques

**Pinch Method** (Soft Lenses)
1. Wash and dry hands
2. Look up
3. Pull down lower eyelid
4. Slide lens down to white of eye
5. Pinch gently between thumb and finger
6. Remove and discard/clean

**Alternative Method**
Use your index finger to slide the lens to outer corner, then pinch.

## Caring for Your Lenses (Non-Dailies)

### Daily Routine

**Every Night:**
1. Wash hands thoroughly
2. Remove lenses
3. Rub with solution (even if "no-rub")
4. Rinse with fresh solution
5. Store in clean case with fresh solution

**Every Morning:**
1. Wash hands
2. Remove lenses from case
3. Rinse with solution
4. Insert lenses
5. Empty and rinse case
6. Leave case open to air dry

### Weekly Maintenance
- Deep clean with protein remover (if recommended)
- Replace lens case every 3 months
- Check lenses for tears or deposits

### Never Do This!
❌ Use tap water on lenses
❌ Sleep in lenses (unless approved)
❌ Swim or shower wearing lenses
❌ Reuse solution
❌ Store lenses in water
❌ Share lenses with others

## Common Beginner Concerns

### "I Can't Touch My Eye!"
It's normal to be hesitant. With practice, it becomes easy. Start by:
- Practicing with artificial tears
- Touching the white part of your eye
- Gradually building confidence

### "Will They Fall Out?"
Modern lenses fit securely and rarely fall out during normal activities.

### "Will They Be Uncomfortable?"
Quality lenses are comfortable once you adapt (usually 1-2 weeks).

### "How Much Do They Cost?"
Costs vary by type:
- Daily disposables: £25-45/month
- Monthly lenses: £15-30/month
- Solutions and accessories: £5-10/month

## Signs to Remove Lenses Immediately

Remove your lenses and contact us if you experience:
- Sudden vision changes
- Eye pain
- Redness
- Light sensitivity
- Discharge
- Feeling of something stuck in eye

## Success Tips

### For Comfortable Wear
1. **Follow wearing schedule** - Don't exceed recommended hours
2. **Stay hydrated** - Drink plenty of water
3. **Use rewetting drops** - Combat dryness
4. **Take breaks** - Remove lenses for screen breaks
5. **Clean thoroughly** - Never skip the cleaning routine

### For Long-Term Health
1. **Never skip check-ups** - Annual exams are essential
2. **Replace on schedule** - Don't overwear lenses
3. **Watch for changes** - Report any issues promptly
4. **Update prescription** - When vision changes
5. **Consider daily disposables** - Healthiest option

## Ready to Try Contact Lenses?

At Eyesite Opticians, we make the transition to contact lenses easy and comfortable. Our expert team provides:

- Comprehensive contact lens fitting
- Wide range of lens options
- Patient training and support
- Competitive pricing
- Ongoing aftercare
- Emergency support

**Book your contact lens consultation today!**

Call: 0113 260 0063
Visit: 198 Selby Rd, Halton, Leeds LS15 0LF

We're here to help you see clearly and comfortably!
    `,
    author: "Emma Thompson",
    date: "2024-11-28",
    category: "Contact Lenses",
    image: "/lens.png",
    readTime: "10 min read",
    tags: ["Contact Lenses", "Beginner Guide", "Eye Care", "Vision"]
  },
  {
    slug: "protect-your-eyes-from-digital-strain",
    title: "Protect Your Eyes from Digital Eye Strain",
    excerpt: "Spending hours on screens? Learn how to prevent digital eye strain and keep your eyes healthy in the digital age.",
    content: `
# Protect Your Eyes from Digital Eye Strain

In today's digital world, most of us spend hours staring at screens – computers, tablets, smartphones, and TVs. This can lead to digital eye strain, also known as computer vision syndrome. Learn how to protect your eyes and stay comfortable.

## What is Digital Eye Strain?

Digital eye strain refers to a group of eye and vision problems resulting from prolonged use of computers, tablets, smartphones, and other digital devices.

### Common Symptoms

**Visual Symptoms:**
- Blurred vision
- Double vision
- Difficulty focusing
- Watery or dry eyes
- Burning or itchy eyes

**Physical Symptoms:**
- Headaches
- Neck and shoulder pain
- Eye fatigue
- Sore, tired eyes

**How Common is It?**
Studies show that 50-90% of people who work on computers experience symptoms of digital eye strain.

## Why Does It Happen?

### Reduced Blinking
We blink less when looking at screens (about 66% less!), leading to dry eyes.

### Poor Lighting
Glare from screens or surrounding light causes eye strain.

### Improper Viewing Distance
Sitting too close or too far from screens.

### Poor Posture
Neck and shoulder tension contributes to eye discomfort.

### Blue Light Exposure
High-energy visible (HEV) blue light from screens may contribute to eye fatigue.

### Uncorrected Vision Problems
Small refractive errors become noticeable during extended screen use.

## The 20-20-20 Rule

The most important habit to adopt:

**Every 20 minutes, look at something 20 feet away for 20 seconds.**

This gives your eye muscles a break and helps reduce fatigue.

### How to Remember
- Set phone/computer reminders
- Use apps like "Eye Care 20 20 20"
- Stick a note on your monitor
- Use browser extensions

## Optimise Your Workspace

### Monitor Position
- **Distance:** 20-26 inches from eyes
- **Height:** Top of screen at or below eye level
- **Angle:** Slight downward gaze of 10-15 degrees

### Lighting
- **Avoid glare:** Position screen perpendicular to windows
- **Use indirect lighting:** Avoid overhead lights directly above
- **Adjust screen brightness:** Match surrounding light levels
- **Use task lighting:** For reading paper documents

### Screen Settings
- **Brightness:** Should match surroundings
- **Contrast:** Adjust for comfortable reading
- **Text size:** Increase if straining to read
- **Color temperature:** Warmer tones in evening

## Blink More Often

Make a conscious effort to blink fully and regularly:
- Remind yourself to blink
- Practice full, complete blinks
- Use lubricating eye drops if needed
- Consider a humidifier in dry environments

## Blue Light Protection

### What is Blue Light?
Short-wavelength light (400-495nm) emitted by digital screens and LED lights.

### Potential Effects
- May contribute to eye strain
- Can disrupt sleep patterns
- Long-term effects still being studied

### Protection Options

**Blue Light Filtering Glasses**
- Reduce blue light exposure
- May improve comfort during screen use
- Available at Eyesite Opticians

**Screen Filters**
- Physical filters for monitors
- Software filters (Night Shift, f.lux)
- Built-in device settings

**Device Settings**
- Enable "Night Mode" in evenings
- Reduce screen brightness
- Use dark mode when available

## Computer Glasses

Special glasses designed for intermediate distances (computer screens):

### Benefits
- Optimised for screen distance (20-26 inches)
- Reduces eye strain
- Can include blue light filtering
- Anti-reflective coating reduces glare

### Who Needs Them?
- Anyone spending 2+ hours daily on computers
- People over 40 (presbyopia makes focusing harder)
- Those experiencing symptoms of digital eye strain

### Available at Eyesite Opticians
We offer customised computer glasses with:
- Precise intermediate distance prescriptions
- Anti-reflective coatings
- Blue light filtering
- Personalised fitting

## Exercises for Eye Strain Relief

### Eye Yoga

**Palming**
1. Rub hands together to warm them
2. Close eyes and cover with palms
3. Relax for 1-2 minutes
4. Repeat several times daily

**Figure Eight**
1. Imagine a large figure 8 (10 feet away)
2. Trace it with your eyes slowly
3. Do clockwise, then counterclockwise
4. Repeat 10 times

**Near and Far Focus**
1. Hold thumb 10 inches away
2. Focus on thumb for 5 seconds
3. Focus on object 10-20 feet away
4. Alternate 10 times

**Eye Rolling**
1. Look up, circle eyes clockwise
2. Repeat 5 times
3. Circle counterclockwise 5 times
4. Helps lubricate eyes

## Lifestyle Adjustments

### Take Regular Breaks
- Stand up every hour
- Walk around
- Stretch neck and shoulders
- Look out windows

### Stay Hydrated
- Drink water throughout the day
- Helps maintain tear production
- Reduces dry eye symptoms

### Adjust Your Diet
Foods rich in:
- **Omega-3 fatty acids:** Fish, flaxseed
- **Lutein and zeaxanthin:** Leafy greens
- **Vitamin A:** Carrots, sweet potatoes
- **Vitamin C:** Citrus fruits

### Get Adequate Sleep
- 7-9 hours per night
- Gives eyes time to rest and recover
- Avoid screens 1 hour before bed

## For Children and Teens

Digital eye strain affects young people too!

### Special Considerations
- Shorter attention spans make breaks harder
- Still developing vision
- Increased myopia (nearsightedness) risk

### Guidelines for Kids
- **Under 2 years:** Avoid screen time
- **2-5 years:** Maximum 1 hour/day
- **6+ years:** Consistent limits
- **All ages:** No screens before bed

### Encourage Outdoor Play
- Reduces myopia risk
- Natural light benefits eyes
- Breaks from screen time

## When to See an Optometrist

Book an appointment if you experience:
- Persistent symptoms despite changes
- Worsening vision
- Frequent headaches
- Eye pain
- Symptoms affecting work or daily life

## Comprehensive Eye Exam

An eye exam at Eyesite Opticians includes:

1. **Vision Assessment** - Checking for refractive errors
2. **Eye Health Check** - Examining internal eye structures
3. **Discussion of Symptoms** - Understanding your digital habits
4. **Prescription Update** - If needed
5. **Recommendation for Computer Glasses** - If appropriate

## Prevention is Key

Don't wait for symptoms to develop:

✅ Schedule regular eye exams
✅ Practice the 20-20-20 rule
✅ Optimise your workspace
✅ Take frequent breaks
✅ Consider computer glasses
✅ Use artificial tears as needed

## Expert Help at Eyesite Opticians

We specialise in managing digital eye strain with:

- Comprehensive vision assessments
- Custom computer glasses
- Blue light filtering lenses
- Workplace ergonomics advice
- Dry eye treatments
- Personalised recommendations

**Protect your eyes today!**

Call: 0113 260 0063
Visit: 198 Selby Rd, Halton, Leeds LS15 0LF

Your eyes work hard for you – take care of them!
    `,
    author: "Dr. Michael Chen",
    date: "2024-12-01",
    category: "Eye Health",
    image: "/eye-exams.png",
    readTime: "9 min read",
    tags: ["Digital Eye Strain", "Screen Time", "Eye Health", "Prevention", "Computer Glasses"]
  }
];

// Helper functions
export function getBlogPostBySlug(slug: string): BlogPost | undefined {
  return blogPosts.find(post => post.slug === slug);
}

export function getAllBlogPosts(): BlogPost[] {
  return blogPosts.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
}

export function getBlogPostsByCategory(category: string): BlogPost[] {
  return blogPosts.filter(post => post.category === category);
}

export function getRelatedPosts(currentSlug: string, limit: number = 3): BlogPost[] {
  const currentPost = getBlogPostBySlug(currentSlug);
  if (!currentPost) return [];
  
  return blogPosts
    .filter(post => 
      post.slug !== currentSlug && 
      post.category === currentPost.category
    )
    .slice(0, limit);
}

export const categories = [
  "All",
  "Eye Health",
  "Eyewear",
  "Contact Lenses",
  "Eye Conditions"
];

